...
##  Examples[🔗](find/?domain=Verso.Genre.Manual.section&name=The-Lean-Language-Reference--Error-Explanations--About___--inferDefTypeFailed--Examples "Permalink")
Implicit Argument Cannot be Inferred
OriginalFixed (type annotation)Fixed (implicit argument)
`def `Failed to infer type of definition `emptyNats``emptyNats := `don't know how to synthesize implicit argument `α`   @[List.nil](Basic-Types/Linked-Lists/#List___nil "Documentation for List.nil") ?m.3 context: ⊢ Type u_1`[] `
```
Failed to infer type of definition `emptyNats`
```

`def emptyNats : [List](Basic-Types/Linked-Lists/#List___nil "Documentation for List") [Nat](Basic-Types/Natural-Numbers/#Nat___zero "Documentation for Nat") :=   [] `
`def emptyNats :=   [List.nil](Basic-Types/Linked-Lists/#List___nil "Documentation for List.nil") (α := [Nat](Basic-Types/Natural-Numbers/#Nat___zero "Documentation for Nat")) `
